...
async fn read_table_to_df(
    ...
    if rows.is_empty() {
        ...
        let columns: Vec<Series> = describe
            ...
            .iter()
            .map(|col| {
                Series::new_empty(col.name(), &type_name_to_polars_dtype(col.type_info().name()))
            })
            .collect();
...
fn generate_foreign_keys(schema: &TableSchema) -> String {
    ...
    for fk in &schema.foreign_keys {
        ...
        let foreign_cols: Vec<String> =
            fk.foreign_columns.iter().map(|c| format!("\"{}\"", c)).collect();
...
fn generate_rebuild_sql(schemas: &[TableSchema], format: &OutputFormat) -> String {
    ...
    sql.push_str("-- Generated by tranche\n");
...
mod tests {
    ...
    fn test_udt_to_sql_type_basic() {
        assert_eq!(udt_to_sql_type(&col("x", "int2", true, None, None, None, None)), "SMALLINT");
        assert_eq!(udt_to_sql_type(&col("x", "int4", true, None, None, None, None)), "INTEGER");
        assert_eq!(udt_to_sql_type(&col("x", "int8", true, None, None, None, None)), "BIGINT");
        assert_eq!(udt_to_sql_type(&col("x", "float4", true, None, None, None, None)), "REAL");
        assert_eq!(udt_to_sql_type(&col("x", "float8", true, None, None, None, None)), "DOUBLE PRECISION");
        assert_eq!(udt_to_sql_type(&col("x", "bool", true, None, None, None, None)), "BOOLEAN");
        assert_eq!(udt_to_sql_type(&col("x", "text", true, None, None, None, None)), "TEXT");
        assert_eq!(udt_to_sql_type(&col("x", "date", true, None, None, None, None)), "DATE");
        assert_eq!(udt_to_sql_type(&col("x", "timestamptz", true, None, None, None, None)), "TIMESTAMPTZ");
        assert_eq!(udt_to_sql_type(&col("x", "uuid", true, None, None, None, None)), "UUID");
        assert_eq!(udt_to_sql_type(&col("x", "jsonb", true, None, None, None, None)), "JSONB");
        assert_eq!(
    ...
    fn test_udt_to_sql_type_serial() {
        let c = col("id", "int4", false, Some("nextval('users_id_seq'::regclass)"), None, None, None);
        assert_eq!(udt_to_sql_type(&c), "SERIAL");

        let c = col("id", "int8", false, Some("nextval('big_id_seq'::regclass)"), None, None, None);
    ...
    fn test_generate_create_table() {
        let schema = TableSchema {
            ...
            columns: vec![
                col("id", "int4", false, Some("nextval('users_id_seq'::regclass)"), None, None, None),
...
mod integration_tests {
    ...
    async fn setup_test_table(pool: &PgPool, name: &str, n_rows: i64) -> String {
        let table_name = format!("tranche_test_{}", name);
    ...
    async fn test_read_table_with_seed_determinism() {
        ...
        let df2 = read_table_to_df(&pool, &table, Some(30), Some(42))
            .await
            .unwrap();
        assert!(df1.frame_equal(&df2), "same seed should produce identical results");
        teardown_test_table(&pool, &table).await;
    }
